...
export class FrameRateUI {
  ...
  display_graph() {
    ...
    this.flag.graph.push({ratio: fpsRatio, fps: this.rawFps});
...
export class AudioAnalyzer {
  ...
  addControlPoint(options) {
    ...
    if (options.bufferPosition > this.bufferLength) {
      console.error(
        control,
        ' : the frequency cannot be superior to the buffer size'
      );
    } else {
      var that = this;
      ...
      if (this.debug) {
        console.log(
          'New audio control point created at buffer position',
          control.frequency
        );
      }
      control._data = {
      ...
      control.shift = function(frequency) {
        if (frequency != undefined) {
          ...
          console.log(
            'Audio control (id : ' +
              control.id +
              ') point shifted at buffer position ',
            frequency
          );
        }
      };
  ...
  updateControlPoint() {
    ...
    for (var i = 0; i < this.controls.length; i++) {
      ...
      control._data.position.x =
        control.frequency * (this.width / 4 / this.bufferLength * 2.5 + 1);
      ...
      ctx.arc(
        control._data.position.x,
        control._data.position.y,
        control._data.radius * 2.5,
        0,
        2 * Math.PI
      );
      ctx.fill();
